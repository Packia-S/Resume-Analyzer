...
# os.environ["DOC_LING_DISABLE_OCR"] = "0"
...
with tab2:
    ...
    skill_data = df["skills"].apply(lambda x: eval(x) if isinstance(x, str) else x)
    all_skills = sorted({s for lst in skill_data for s in lst})

    selected = st.multiselect("Select skills", all_skills)

    if selected:
        filtered_df = df[df["skills"].apply(lambda s: all(x in s for x in selected))]
    else:
        filtered_df = df
