# ===== VERY FIRST LINES: ensure writable artifacts path BEFORE any docling/rapidocr imports =====
import os
import sys
...
if sys.platform.startswith("linux"):
    base_artifacts = Path("/tmp/docling_models")
else:
    base_artifacts = Path(tempfile.gettempdir()) / "docling_models"

base_artifacts.mkdir(parents=True, exist_ok=True)

# Ensure docling/rapidocr use a writable path (Streamlit Cloud allows /tmp)
os.environ["DOCLING_ARTIFACTS_PATH"] = str(base_artifacts)
os.environ["DOCLING_SERVE_ARTIFACTS_PATH"] = str(base_artifacts)
os.environ["RAPIDOCR_HOME"] = str(base_artifacts)

print(f"[startup] Docling artifacts path -> {base_artifacts}", flush=True)
# ============================================================================================


# === imports (normal app imports after startup snippet) ===
import streamlit as st
from PIL import Image
# removed DoclingLoader import to avoid accidental OCR init; we still import ExportType when needed
...
# (Optional) if icon path is not available on server, this may error on startup.
# You can replace with a local path included in the repo or remove setting page_icon.
try:
    ...
    if uploaded_file:
        ...
        if st.button("Convert"):
            with st.spinner("Extracting Information..."):
                ...
                # -------------------------
                # Convert using docling WITHOUT OCR
                # -------------------------
                try:
                    pipeline_opts = PdfPipelineOptions(do_ocr=False)
                    converter = DocumentConverter(pipeline_options=pipeline_opts)
                    conv_gen = converter.convert(source=temp_path, export_type=ExportType.MARKDOWN)
                    conv_res = next(conv_gen)
                except StopIteration:
                    st.error("Docling returned no conversion results.")
                    raise
                except Exception as e:
                    st.error(f"Error during docling conversion: {e}")
                    raise

                # simple extraction (common attribute names)
                resume_text = (
                    getattr(conv_res, "page_content", None)
                    or getattr(conv_res, "text", None)
                    or getattr(conv_res, "output", None)
                    or getattr(conv_res, "content", None)
                    or str(conv_res)
                )
...
with tab2:
    ...
    if os.path.exists(csv_file):
        ...
        data = pd.DataFrame(columns=["fullname","email_id","phone_number","current_location","designation","technical_skills","interpersonal_skills","year_of_experience","current_ctc","current_company","expected_ctc","linkedin_url","github_url","portfolio_project_urls","summary","certifications","skills"])
